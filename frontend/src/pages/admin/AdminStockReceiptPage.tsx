...
import { Plus, Trash2, Calculator, AlertTriangle, TrendingUp, Truck, Receipt } from 'lucide-react'
...
export const AdminStockReceiptPage: React.FC = () => {
  ...
  const [profitSettings, setProfitSettings] = useState<ProfitSettings>(() => {
    try {
      ...
      return saved ? { ...DEFAULT_PROFIT_SETTINGS, ...JSON.parse(saved) } : DEFAULT_PROFIT_SETTINGS
    } catch {
  ...
  const updateSettings = (patch: Partial<ProfitSettings>) => {
    ...
    localStorage.setItem('agroreserve_profit_settings', JSON.stringify(updated))
  }
  ...
  const totalAmount =
    watchItems?.reduce((sum, item) => sum + (item.quantity || 0) * (item.purchase_price || 0), 0) ||
    0
  ...
  const profitResults = useMemo((): ProfitCalcResult[] => {
    ...
    return watchItems.map((item) => {
      ...
      const suggestedPrice = purchasePrice * (1 + profitSettings.markupPercent / 100)

      const revenue = sellPrice * qty
      const usnAmount = profitSettings.includeUsn ? revenue * profitSettings.usnRate : 0
      const ndsAmount = profitSettings.includeNds ? revenue * profitSettings.ndsRate : 0

      // Распределяем расходы пропорционально доле позиции в общей закупке
      const costShare =
        totalPurchase > 0
          ? ((profitSettings.transportCost + profitSettings.otherCosts) * purchaseTotal) /
            totalPurchase
          : 0

      const netProfit = revenue - usnAmount - ndsAmount - purchaseTotal - costShare
    ...
    <div className="space-y-5 p-6">
      ...
      {showForm && (
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <h2 className="mb-4 text-base font-semibold text-gray-900">Новый приход</h2>
          <form onSubmit={handleSubmit((data) => saveReceipt(data))} className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <Input
                ...
                {...register('date')}
              />
              <Input label="Номер накладной" placeholder="НК-001" {...register('invoice_number')} />
            </div>
                  ...
                  return (
                    <div key={field.id} className="space-y-1">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="w-24 pt-2.5 text-right text-sm font-semibold text-gray-900">
                          {formatPrice(
                      ...
                      {showCalculator &&
                        ...
                        watchItems?.[index]?.purchase_price > 0 &&
                        watchItems?.[index]?.product_id && (
                          <div className="ml-0 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs sm:ml-4">
            ...
            {showCalculator && (
              <div className="space-y-4 rounded-xl border border-gray-200 bg-gray-50 p-4">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  ...
                  <div>
                    <label className="mb-1 block flex items-center gap-1 text-xs font-medium text-gray-500">
                      <Receipt className="h-3.5 w-3.5" /> Другие расходы, ₽
                    </label>
                    <input
                      type="number"
                      step="100"
                ...
                <div className="flex flex-wrap gap-4">
                  <label className="flex cursor-pointer items-center gap-2">
                    <input
                      ...
                      onChange={(e) => updateSettings({ includeUsn: e.target.checked })}
                  ...
                  <label className="flex cursor-pointer items-center gap-2">
                    <input
                      ...
                      onChange={(e) => updateSettings({ includeNds: e.target.checked })}
                      className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                    />
                ...
                {lowMarginItems.length > 0 && (
                  <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
                    ...
                    <div className="text-sm text-amber-800">
                      <span className="font-semibold">Внимание:</span> {lowMarginItems.length}{' '}
                      {lowMarginItems.length === 1
                        ? 'позиция'
                        : lowMarginItems.length < 5
                          ? 'позиции'
                          : 'позиций'}{' '}
                      с маржой ниже {profitSettings.minMarginPercent}%. Рассмотрите повышение цены
                      продажи.
                    </div>
                  </div>
                )}
                    ...
                    <table className="w-full text-sm">
                      <thead className="border-b border-gray-200 bg-white">
                        <tr>
                          ...
                          {(profitSettings.transportCost > 0 || profitSettings.otherCosts > 0) && (
                            <th className="px-3 py-2 text-right text-xs font-semibold text-gray-500">
                              Расходы
                            </th>
                          )}
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {profitResults.map((result, index) => {
                          if (!watchItems?.[index]?.product_id || result.sellPrice === 0)
                            return null
                          const product = productsMap.get(watchItems[index].product_id)
                          const isLow = result.marginPercent < profitSettings.minMarginPercent

                          return (
                            <tr key={index} className={isLow ? 'bg-amber-50/50' : ''}>
                ...
                {profitTotals.revenue > 0 && (
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <div className="rounded-lg border border-gray-200 bg-white p-3 text-center">
                      <div className="mb-1 text-xs text-gray-500">Выручка</div>
                      <div className="text-lg font-bold text-gray-900">
                        {formatPrice(profitTotals.revenue)}
                      </div>
                    </div>
                    <div className="rounded-lg border border-gray-200 bg-white p-3 text-center">
                      <div className="mb-1 text-xs text-gray-500">Расходы всего</div>
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-3 text-center">
                      <div className="mb-1 text-xs text-gray-500">Чистая прибыль</div>
                      <div
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-3 text-center">
                      <div className="mb-1 text-xs text-gray-500">Маржа</div>
                      <div
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        ...
        {isLoading ? (
          ...
          <div className="py-8 text-center text-sm text-gray-400">Приходов нет</div>
        ) : (
          <table className="w-full text-sm">
