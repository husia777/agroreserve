...
const settingsSchema = z.object({
  ...
  company_inn: z.string().min(10, 'ИНН минимум 10 символов').max(12, 'ИНН максимум 12 символов'),
  ...
  tax_rate: z.number({ invalid_type_error: 'Введите ставку налога' }).min(0).max(100),
  working_hours: z.string().min(3, 'Укажите часы работы'),
  delivery_slots: z.array(z.object({ slot: z.string().min(1, 'Введите временной слот') })),
})
...
const Section: React.FC<{
  ...
  <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
    <div className="flex items-center gap-3 border-b border-gray-100 px-5 py-4">
      <div className="rounded-lg bg-green-50 p-1.5 text-green-600">{icon}</div>
      <div>
...
const AdminSettingsPage: React.FC = () => {
  ...
  const {
    ...
    defaultValues: {
      ...
      delivery_slots: [{ slot: '08:00-11:00' }, { slot: '11:00-14:00' }, { slot: '14:00-17:00' }],
    },
  })
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div
            className={cn(
              'flex h-24 w-24 items-center justify-center overflow-hidden rounded-xl border-2 border-dashed bg-gray-50',
              logoPreview ? 'border-green-400' : 'border-gray-300',
            )}
          >
            {logoPreview ? (
              <img src={logoPreview} alt="Логотип" className="h-full w-full object-contain p-2" />
            ) : (
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-400">PNG, JPG, SVG до 2 МБ. Рекомендуется 200×200 px</p>
          </div>
      ...
      >
        <div className="space-y-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 py-1 text-sm text-green-600 hover:text-green-700"
          >
      ...
      >
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            />
            {errors.tax_rate && (
              <p className="mt-1 text-xs text-red-500">{errors.tax_rate.message}</p>
            )}
            <p className="mt-1 text-xs text-gray-400">Применяется для расчёта налога в P&L</p>
          </div>
