...
export const AdminBatchesPage: React.FC = () => {
  ...
  const filtered = React.useMemo(() => {
    ...
    return batches.filter((b) => b.product_name.toLowerCase().includes(q))
  }, [batches, search])
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-gray-100 bg-gray-50">
              <tr>
                ...
                <th className="hidden px-5 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-500 md:table-cell">
                  FIFO
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filtered.map((batch: Batch, idx) => {
                ...
                const isFifoNext =
                  !batch.is_exhausted && idx === filtered.findIndex((b) => !b.is_exhausted)

                return (
                  <tr
                    key={batch._id}
                    className={`transition-colors hover:bg-gray-50/50 ${rowClass}`}
                  >
                    <td className="px-5 py-3">
                      <div className="font-medium text-gray-900">{batch.product_name}</div>
                      <div className="font-mono text-xs text-gray-400">{batch._id.slice(-6)}</div>
                    </td>
                    <td className="px-5 py-3 text-right text-gray-600">{batch.qty_initial}</td>
