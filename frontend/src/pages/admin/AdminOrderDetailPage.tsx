...
export const AdminOrderDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    if (order?.items) {
      ...
      for (const item of order.items) {
        initial[item.product_id] = item.actual_qty !== null ? String(item.actual_qty) : ''
      }
  ...
  if (!order)
    return (
      <div className="p-6 text-center text-gray-500">
        Заказ не найден.{' '}
        <Link to="/admin/orders" className="text-primary-600">
          К списку
        </Link>
      </div>
    )
  ...
  return (
    <div className="space-y-5 p-6">
      <Breadcrumbs
        items={[{ label: 'Заказы', href: '/admin/orders' }, { label: order.order_number }]}
        showHome={false}
      />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">{formatDateTime(order.created_at)}</p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-gray-100 bg-gray-50">
                  <tr>
                    ...
                    <th className="min-w-[120px] px-5 py-2.5 text-right text-xs font-semibold uppercase text-gray-500">
                      Факт
                    </th>
                    <th className="px-5 py-2.5 text-right text-xs font-semibold uppercase text-gray-500">
                      Цена
                ...
                <tbody className="divide-y divide-gray-100">
                  {order.items?.map((item) => {
                    ...
                    const factTotal =
                      !isNaN(parsedQty) && parsedQty >= 0 ? parsedQty * item.price : item.total
                        ...
                        <td className="px-5 py-3 text-right">
                          {isEditable ? (
                            <input
                              ...
                              className={`w-24 rounded-lg border px-2 py-1.5 text-right text-sm focus:outline-none focus:ring-2 focus:ring-primary-300 ${
                                hasDiff
              ...
              <div className="space-y-2 text-sm">
                ...
                {order.client?.organization && (
                  <div className="font-medium text-primary-600">
                    {order.client.organization.name}
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="space-y-4 lg:col-span-1">
          ...
          {order.payment_method === 'prepayment' && order.payment_status !== PaymentStatus.PAID && (
            <div className="rounded-xl border border-yellow-200 bg-yellow-50 p-5">
              <h2 className="mb-2 font-semibold text-yellow-800">Ожидает оплаты</h2>
          ...
          {nextStatuses.length > 0 && (
            <div className="rounded-xl border border-gray-200 bg-white p-5">
              <h2 className="mb-3 font-semibold text-gray-900">Сменить статус</h2>
              <div className="space-y-3">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full resize-none rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300"
                  />
                </div>
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
              <div className="flex gap-2">
                ...
                <div className="text-sm text-amber-800">
                  <p className="mb-1 font-medium">Ввод фактического веса</p>
                  <p className="text-amber-700">
                    Введите реальный вес каждой позиции после взвешивания. Сумма пересчитается
                    автоматически.
                  </p>
                </div>
              </div>
          ...
          {/* Таймлайн */}
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            <h2 className="mb-4 font-semibold text-gray-900">История статусов</h2>
