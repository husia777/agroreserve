...
import { User, UserStatus, ClientType } from '@/types'
import { formatPrice, formatDate, formatPhone } from '@/utils/format'
import { cn } from '@/utils/cn'
import { generateContract } from '@/api/admin'
...
const statusColor = (status: UserStatus) => {
  switch (status) {
    case UserStatus.APPROVED: return 'green'
    case UserStatus.PENDING: return 'yellow'
    case UserStatus.REJECTED: return 'red'
    case UserStatus.BLOCKED: return 'red'
    default: return 'gray'
  }
}
...
const statusLabel = (status: UserStatus) => {
  switch (status) {
    case UserStatus.APPROVED: return 'Одобрен'
    case UserStatus.PENDING: return 'Ожидает'
    case UserStatus.REJECTED: return 'Отклонён'
    case UserStatus.BLOCKED: return 'Заблокирован'
...
const clientTypeLabel = (type: ClientType) => {
  switch (type) {
    case ClientType.OOO: return 'ООО'
    case ClientType.INDIVIDUAL: return 'Физлицо'
    case ClientType.IP: return 'ИП'
    default: return type
  }
}
...
const ClientRow: React.FC<{
  ...
  const debtPercent = client.credit_limit > 0
          ...
          <td colSpan={6} className="px-4 pb-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-2">
              ...
              <div className="space-y-1">
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Контакты</div>
                ...
                {client.delivery_address && (
                  <div className="text-xs text-gray-500 mt-1">{client.delivery_address}</div>
                )}
              </div>
              ...
              {client.organization && (
                <div className="space-y-1">
                  <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Организация</div>
                  <div className="flex items-center gap-1.5 text-sm text-gray-700">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className={cn(
                    'text-xs font-medium',
                    debtPercent >= 90 ? 'text-red-600' : debtPercent >= 70 ? 'text-amber-600' : 'text-gray-500'
                  )}>
                ...
                <div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    className={cn(
                      ...
                      debtPercent >= 90 ? 'bg-red-500' : debtPercent >= 70 ? 'bg-amber-500' : 'bg-green-500'
                    )}
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3">
              {client.status === UserStatus.PENDING && (
                <>
                  <Button
                    variant="primary"
                    size="sm"
                    onClick={(e) => { e.stopPropagation(); onApprove(client.id) }}
                    loading={approving}
                  ...
                  <Button
                    ...
                    onClick={(e) => { e.stopPropagation(); onReject(client) }}
              ...
              {client.status === UserStatus.APPROVED && (
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={(e) => { e.stopPropagation(); onSetLimit(client) }}
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={(e) => { e.stopPropagation(); onGenerateContract(client) }}
...
const AdminClientsPage: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getAdminClients({ search, status: statusFilter || undefined, page, per_page: 20 }),
  })
    ...
    <div className="space-y-6">
        ...
        {stats.pending > 0 && (
          <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 text-amber-800 rounded-lg px-3 py-2">
            ...
            <span className="text-sm font-medium">
              {stats.pending} заявок ожидают модерации
            </span>
          </div>
        )}
      </div>
                ...
                <tbody>
                  {data.items.map((client) => (
                    <ClientRow
                      ...
                      onReject={(c) => { setRejectModal(c); rejectForm.reset() }}
          ...
          >
            <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-lg p-3">
              ...
              <div>
                ...
                <p className="text-sm text-red-700 mt-0.5">{rejectModal.full_name}</p>
              </div>
            </div>
      ...
      {contractModal && (
        <Modal
          ...
          onClose={() => { setContractModal(null); setContractType('supply') }}
        ...
        >
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Тип договора</label>
              <div className="space-y-2">
                {[
                  { value: 'supply', label: 'Договор поставки', desc: 'Стандартный договор на поставку продукции' },
                  { value: 'supply_44fz', label: 'Госконтракт 44-ФЗ', desc: 'Прямая закупка до 600 000 руб' },
                  { value: 'agency', label: 'Агентский договор', desc: 'Договор на агентские услуги' },
                ].map(opt => (
                  <label
                    key={opt.value}
                    className={cn(
            ...
            <div className="flex gap-3 pt-2">
              <Button
                variant="ghost"
                className="flex-1"
                onClick={() => { setContractModal(null); setContractType('supply') }}
              >
                Отмена
              </Button>
              <Button
