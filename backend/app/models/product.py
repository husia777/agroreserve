...
class Category(Document):
    ...
    parent_id: Optional[Link["Category"]] = Field(
        None, description="Родительская категория")

    # ── Сортировка ────────────────────────────────────────────
    sort_order: int = Field(
        0, description="Порядок отображения (меньше = выше)")
...
class Product(Document):
    ...
    unit: ProductUnit = Field(
        ProductUnit.KG, description="Основная единица измерения")
    ...
    price_wholesale: float = Field(..., ge=0,
                                   description="Оптовая цена за единицу (₽)")
    # Розничная цена (для физических лиц)
    price_retail: float = Field(..., ge=0,
                                description="Розничная цена за единицу (₽)")
    # Закупочная цена (себестоимость, только для администратора)
    cost_price: float = Field(
        0.0, ge=0, description="Закупочная цена (себестоимость)")

    # ── Заказ ─────────────────────────────────────────────────
    min_order_qty: float = Field(
        1.0, ge=0, description="Минимальное количество заказа")
    order_step: float = Field(
        0.5, ge=0, description="Шаг изменения количества")

    # ── Склад ─────────────────────────────────────────────────
    stock_qty: float = Field(
        0.0, ge=0, description="Текущий остаток на складе")
    min_stock_qty: float = Field(
        0.0, ge=0, description="Минимальный остаток (алерт при достижении)")

    # ── Медиа ─────────────────────────────────────────────────
    images: List[str] = Field(default_factory=list,
                              description="Список URL изображений")
    # Ссылки на связанные сертификаты
    certificate_ids: List[Link["Certificate"]] = Field(
