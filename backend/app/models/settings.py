...
class SystemSettings(Document):
    ...
    singleton_key: str = Field(
        "main", description="Всегда 'main' — один документ")

    # ── Реквизиты компании ────────────────────────────────────
    company_name: str = Field(
        "ИП Наимов Хусейн Вохиджонович", description="Название ИП/организации")
    ...
    logo_url: Optional[str] = Field(
        None, description="URL логотипа (для PDF документов)")
    stamp_url: Optional[str] = Field(
        None, description="URL изображения печати")
    ...
    min_order_amount_b2b: float = Field(
        500.0, ge=0, description="Минимальная сумма B2B заказа (₽)")
    # Количество дней для подтверждения заказа (после этого — автоотмена)
    order_confirmation_days: int = Field(
        2, description="Дней до автоотмены неподтверждённого заказа")
    # Срок оплаты по умолчанию
    default_payment_days: int = Field(
        14, description="Срок оплаты по умолчанию (дней)")
